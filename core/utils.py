...
@torch.no_grad()
def translate_using_latent(nets, args, x_src, y_trg_list, z_trg_list, psi, filename=None):
    N, C, H, W = x_src.size()
...
@torch.no_grad()
def cycle_equal_using_latent(nets, args, x_src, y_src, y_trg_list, z_trg_list, psi, filename):
    N, C, H, W = x_src.size()
    latent_dim = z_trg_list[0].size(1)
    x_concat = [x_src]
    masks = nets.fan.get_heatmap(x_src) if args.w_hpf > 0 else None
    ...
    s_src = nets.style_encoder(x_src, y_src)
    y_equal = torch.LongTensor([args.num_domains] * len(y_src)).to(x_src.device)
    ...
    for i, y_trg in enumerate(y_trg_list):
        ...
        for z_trg in z_trg_list:
            s_trg = nets.mapping_network(z_trg, y_trg)
            s_trg_equal = nets.mapping_network_equal(z_trg, mapping_network=nets.mapping_network)
...
@torch.no_grad()
def debug_image_equal(nets, args, inputs, step):
    x_src, y_src = inputs.x_src, inputs.y_src
    x_ref, y_ref = inputs.x_ref, inputs.y_ref
    ...
    # for psi in [0.5, 0.7, 1.0]:
    psi = 1.0
    filename = ospj(args.sample_dir, '%06d_equal_latent_psi_%.1f.jpg' % (step, psi))
    # translate_using_latent(nets, args, x_src, y_trg_list, z_trg_list, psi, filename)
    cycle_equal_using_latent(nets, args, x_src, y_src, y_trg_list, z_trg_list, psi, filename)
